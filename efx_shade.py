...
# PROC functions
# bgimgを切り抜いてfgimgの前に貼る
def add_foreimage(fgimage, mask, bgimg):
    image = add_silhouette(bgimage, mask, fgimg)
    return image


# 前景を切り抜いて影付きで貼る(numpy版)
def add_silhouette(fgimage, mask, bgimg=None,
    ...
    if fgimage is not None:
        foreground = fgimage.convert("RGBA")
        W, H = foreground.size
    else:
