...
- stoi(s, default=0, lo=None, hi=None, multi=False)

  - 文字列を安全に数値に変換、変換エラー時はデフォルト値を返す
  - lo,hi で値域を指定
  - 0x,0o,0bのプレフィクス対応、0.0や 1e4 などの実数にも対応

  - multi=Trueにすると listもしくはカンマ区切り文字列を数値リストにして返す
    - 無指定(multi=False)だと先頭の値のみ返却
    - 例 multi時)  [[1, 2, 3],  ['4.0', '5', '8'], '7e3,8,9'] -> [[1,2,3].[4.0,5,6],[7000,8,9]] 
